...
KMSDRM_VideoInit(_THIS)
{
    ...
    if (mode_override) {
        dispdata->mode = dispdata->conn->modes[SDL_atoi(mode_override)];
    ...
    if (mode_line_override && !mode_override) {
       ...
       override_mode = connector_find_mode(dispdata->conn, hint_video_mode, hint_vrefresh);
       if (override_mode) {
            ...
            SDL_LogWarn(SDL_LOG_CATEGORY_VIDEO, "Cannot find a suitable video mode on connector %d for mode %s, using default",
                dispdata->conn->connector_id, mode_line_override);
        }
    }
